...
public class GeografijaDAO {
    ...
    private GeografijaDAO() {
        try {
           ...
           // e.printStackTrace();
//            System.out.println("Napravi se baza");
        }

        try {
//            Statement s = conn.createStatement();
//            s.execute("select nadmorskaVisina from grad");
            glavniGradUpit = conn.prepareStatement("SELECT grad.id, grad.naziv, grad.broj_stanovnika, grad.drzava, grad.nadmorskaVisina FROM grad, drzava WHERE grad.drzava=drzava.id AND drzava.naziv=?");
        } catch (SQLException e) {
            regenerisiBazu();
            try {
                glavniGradUpit = conn.prepareStatement("SELECT grad.id, grad.naziv, grad.broj_stanovnika, grad.drzava, grad.nadmorskaVisina FROM grad, drzava WHERE grad.drzava=drzava.id AND drzava.naziv=?");
        ...
        try {
            ...
            promijeniGradUpit = conn.prepareStatement("UPDATE grad SET naziv=?, broj_stanovnika=?, drzava=?, nadmorskaVisina=? WHERE id=?");
        } catch (SQLException e) {
            e.printStackTrace();
        }
...
//            Statement s = conn.createStatement();
//            s.executeUpdate("UPDATE grad SET naziv=\'"+grad.getNaziv()+"\', broj_stanovnika="+grad.getBrojStanovnika()+", drzava="+grad.getDrzava().getId()+", nadmorskaVisina="+grad.getNadmorskaVisina()+" WHERE id="+grad.getId());
            promijeniGradUpit.executeUpdate();
            System.out.println(grad.getNadmorskaVisina());;
        } catch (SQLException e) {
//            e.printStackTrace();
